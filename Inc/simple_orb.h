...
typedef struct {
    ...
    ORB_ERR_T(*delay)(void);                           /**< 忙等延迟函数（仅在使用 sequence lock 模式时有效） */

    void * mutex;                                      /**< Mutex 对象指针（仅在使用 mutex 模式时有效） */
    void(*take)(void * mutex);                         /**< P 操作/获取锁回调 */
    void(*give)(void * mutex);                         /**< V 操作/释放锁回调 */
} ORB_HANDLE_T;
...
typedef struct {
    ...
    void * mail;                                     /**< 用户自定义消息缓冲区，可选（NULL 表示不使用 send/receive 回调） */
    void(*send)(void * mail);                        /**< 发送回调：当使用阻塞模式时，在阻塞等待过程中调用 */
    void(*receive)(void * mail);                     /**< 接收回调：当使用阻塞模式时，在阻塞等待前调用（清空上一轮消息）*/
